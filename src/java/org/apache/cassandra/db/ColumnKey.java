...
public class ColumnKey implements Named
{
    ...
    {
        FBUtilities.writeShortByteArray(StorageService.getPartitioner().convertToDiskFormat(dk), dos);

        dos.writeByte((byte)names.length);
        for (byte[] name : names)
            FBUtilities.writeShortByteArray(name, dos);
    }

    public static ColumnKey deserialize(DataInput dis) throws IOException
    {
        DecoratedKey dk = StorageService.getPartitioner().convertFromDiskFormat(FBUtilities.readShortByteArray(dis));

        byte nameCount = dis.readByte();
        byte[][] names = new byte[nameCount][];
        for (int i = 0; i < nameCount; i++)
            names[i] = FBUtilities.readShortByteArray(dis);
